...
func (s *SmartContract) Init(APIstub shim.ChaincodeStubInterface) peer.Response {
	...
	Element = Element{
		Type:     "task",
		ID:       "Task_085tf8r",
		Name:     "Pre-treated Raw Materials",
		Token:    0,
		XORtoken: []string{},
		ANDtoken: map[string]int{},
		Children: []string{"Task_0j5dvjy"},
	}
	...
	Element = Element{
		...
		XORtoken: []string{},
		ANDtoken: map[string]int{},
		Children: []string{"EndEvent_05e5is6"},
	}
	Functions[event.Name] = event.ID
	eventAsBytes, _ = json.Marshal(event)
	APIstub.PutState(event.ID, eventAsBytes)
	...
	Element = Element{
		Type:     "START",
		ID:       "StartEvent_1oypnw9",
		Name:     "undefined",
		Token:    0,
		XORtoken: []string{},
		ANDtoken: map[string]int{},
		Children: []string{"Task_085tf8r"},
	}
...
func (s *SmartContract) interchainGet(APIstub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	taskEvent2, err := s.GetEvent(APIstub, Functions["Pre-treated Raw Materials"])
	s.DoTask(APIstub, taskEvent2)
	taskEvent3, err := s.GetEvent(APIstub, Functions["Processed Goods"])
	s.DoTask(APIstub, taskEvent3)
	taskEvent4, err := s.GetEvent(APIstub, Functions["Testing Goods"])
