...
func (s *SmartContract) Init(APIstub shim.ChaincodeStubInterface) peer.Response {
	...
	Element = Element{
		...
		Name:     "Issue Invoice",
		Token:    0,
        Functions[IssueInvoice] = Issue,
        Args map[string]string{},
		XORtoken: []string{},
		ANDtoken: map[string]int{},
		Children: []string{"Task_168mmdo"},
	}
	...
	Element = Element{
		...
		Token:    0,
        Functions[ValidateInvoice] = Validate,
        Args map[string]string{},
		XORtoken: []string{},
		ANDtoken: map[string]int{},
		Children: []string{"EndEvent_0ti6r5l"},
	}
	Functions[event.Name] = event.ID
	eventAsBytes, _ = json.Marshal(event)
	APIstub.PutState(event.ID, eventAsBytes)
	...
	Element = Element{
		Type:     "START",
		ID:       "StartEvent_0ca4rit",
		Name:     "undefined",
		Token:    0,
		XORtoken: []string{},
		ANDtoken: map[string]int{},
		Children: []string{"Task_0ialkjw"},
	}
...
func (s *SmartContract) interchainGet(APIstub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	taskEvent2, err := s.GetEvent(APIstub, Functions["Issue Invoice"])
	s.DoTask(APIstub, taskEvent2)
	taskEvent3, err := s.GetEvent(APIstub, Functions["Validate Invoice"])
