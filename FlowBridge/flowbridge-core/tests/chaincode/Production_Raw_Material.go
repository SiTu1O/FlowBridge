...
func (s *SmartContract) Init(APIstub shim.ChaincodeStubInterface) peer.Response {
	...
	Element = Element{
		Type:     "task",
		ID:       "Task_1b4wqcn",
		Name:     "Collecting Raw Materials",
		Token:    0,
		XORtoken: []string{},
		ANDtoken: map[string]int{},
		Children: []string{"Task_16wr3rr"},
	}
	...
	Element = Element{
		Type:     "task",
		ID:       "Task_16wr3rr",
		Name:     "Handling Raw Materials",
		Token:    0,
		XORtoken: []string{},
		ANDtoken: map[string]int{},
		Children: []string{"Task_1lmrzxt"},
	}
	Functions[event.Name] = event.ID
	eventAsBytes, _ = json.Marshal(event)
	APIstub.PutState(event.ID, eventAsBytes)
	...
	Element = Element{
		...
		XORtoken: []string{},
		ANDtoken: map[string]int{},
		Children: []string{"EndEvent_10n0jly"},
	}
	Functions[event.Name] = event.ID
	eventAsBytes, _ = json.Marshal(event)
	APIstub.PutState(event.ID, eventAsBytes)
	...
	Element = Element{
		Type:     "START",
		ID:       "StartEvent_07x56it",
		Name:     "undefined",
		Token:    0,
		XORtoken: []string{},
		ANDtoken: map[string]int{},
		Children: []string{"Task_1b4wqcn"},
	}
...
func (s *SmartContract) interchainGet(APIstub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	taskEvent2, err := s.GetEvent(APIstub, Functions["Collecting Raw Materials"])
	s.DoTask(APIstub, taskEvent2)
	taskEvent3, err := s.GetEvent(APIstub, Functions["Handling Raw Materials"])
	s.DoTask(APIstub, taskEvent3)
	taskEvent4, err := s.GetEvent(APIstub, Functions["Packaging Raw Materials"])
	s.DoTask(APIstub, taskEvent4)
