        ...
        ProcessContract.new('0x857133c5C69e6Ce66F7AD46F200B9B3573e77582',
            ...
            (err, contract) => {
                if (err) {
                    ...
                    instances.set(checksumAddress, [modelInfo.entryContractName, modelInfo]);
                    var workListAdr = contract.getWorkListAddress.call().toString();
                    workListInstances.set(workListAdr, checksumAddress);
                    ...
                    contract.allEvents(
                        ...
                        function(error,event){
                            console.log('==============================================================================================');
                            console.log("RECEIVED EVENT FROM CONTRACT AT ADDRESS: ", checksumAddress.toString());
                            console.log('----------------------------------------------------------------------------------------------');
                            console.log(event);
                            console.log('----------------------------------------------------------------------------------------------');
                    ...
                    console.log('CONTRACT CREATED !!! AT ADDRESS: ', checksumAddress);
                    console.log('WORKITEMS ADDRESS: ', contract.getWorkListAddress.call());
                    console.log('----------------------------------------------------------------------------------------------');
                    res.status(201).send({ address: checksumAddress })
                }
            }
        );
    } else
...
models.post('/workitems/:workListAddress/:reqId', (req, res) => {
    ...
    if (validExecution && enabledTasks.has(activityId) && workListInstances.has(workListAddress) && instances.has(workListInstances.get(workListAddress))) {
        ...
        let modelInfoNameabi =  modelInfo.name + "_WorkList";
        console.log('----------------------------------------------------------------------------------------------');
        ...
        result = instance[activityName + '_callback'].apply(this, realParameters.concat({ from: web3.eth.accounts[0], gas: 4700000 }));
