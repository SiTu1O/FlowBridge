...
let collectControlFlowInfo = (proc: any, globalNodeMap: Map<string, any>,
    globalControlFlowInfo: Array<ControlFlowInfo>,userParamMap: Map<string, any>): ControlFlowInfo => {
    ...
    for (let subprocess of proc.flowElements.filter((e) => is(e, "bpmn:SubProcess"))) {
        let subprocessControlFlowInfo =
            collectControlFlowInfo(subprocess, globalNodeMap, globalControlFlowInfo,userParamMap);
    ...
    if (proc.documentation) {
        if(userParamMap.get('cross_param1'))
        {
            let cross_param1 = userParamMap.get('cross_param1');
            let cross_function = userParamMap.get('cross_function');
            proc.documentation[0].text = proc.documentation[0].text.replace(/crossChainFunc\s*=\s*''/, "crossChainFunc = '" + cross_function + "'");
            proc.documentation[0].text = proc.documentation[0].text.replace(/crossChainParam1\s*=\s*''/, "crossChainParam1 = '" + cross_param1 + "'");
        }
        if(userParamMap.get('HE_model')) 
        {
            let HE_model = userParamMap.get('HE_model');
            proc.documentation[0].text = proc.documentation[0].text.replace(/HEModel\s*=\s*''/, "HEModel = '" + HE_model + "'");
        }       
        if(userParamMap.get('risc0_filename')) 
        {
            let risc0_filename = userParamMap.get('risc0_filename');
            let risc0_param1 =  userParamMap.get('risc0_param1');
            let risc0_param2 = userParamMap.get('risc0_param2');
            proc.documentation[0].text = proc.documentation[0].text.replace(/risc0FileName\s*=\s*''/, "risc0FileName = '" + risc0_filename + "'");
            proc.documentation[0].text = proc.documentation[0].text.replace(/risc0Param1\s*=\s*''/, "risc0Param1 = '" + risc0_param1 + "'");
            proc.documentation[0].text = proc.documentation[0].text.replace(/risc0Param2\s*=\s*''/, "risc0Param2 = '" + risc0_param2 + "'");
        }   
        console.log(proc.documentation[0].text);
...
export let parseModel = (modelInfo: ModelInfo) => new Promise<void>((resolve, reject) => {
    parseBpmn(modelInfo.bpmn).then((definitions: any) => {
        modelInfo.solidity = '// SPDX-License-Identifier: SimPL-2.0\n pragma solidity >=0.5.6;\n  pragma experimental ABIEncoderV2;\n';
        modelInfo.controlFlowInfoMap = new Map();

        let userParamMap: Map<string, any> = new Map();
        const xmlDoc = parser.parseFromString(modelInfo.bpmn, 'text/xml');
        const serviceTaskNodeList = xmlDoc.getElementsByTagName('bpmn:serviceTask');
        let cross_param1;
        let cross_function;
        for (let i = 0; i < serviceTaskNodeList.length; i++) {
            const serviceTaskElement = serviceTaskNodeList[i];
            if(serviceTaskElement.getAttribute('cross_param1'))
            {
                cross_param1 = serviceTaskElement.getAttribute('cross_param1');
                cross_function = serviceTaskElement.getAttribute('cross_function');
                userParamMap.set('cross_param1',cross_param1);
                userParamMap.set('cross_function',cross_function);
            }
            if(serviceTaskElement.getAttribute('HE_model')){
                const HE_model = serviceTaskElement.getAttribute('HE_model');
                userParamMap.set('HE_model',HE_model);
            }
            if(serviceTaskElement.getAttribute('risc0_filename')){
                const risc0_filename = serviceTaskElement.getAttribute('risc0_filename');
                const risc0_param1 = serviceTaskElement.getAttribute('risc0_param1');
                const risc0_param2 = serviceTaskElement.getAttribute('risc0_param2');
                userParamMap.set('risc0_filename',risc0_filename);
                userParamMap.set('risc0_param1',risc0_param1);
                userParamMap.set('risc0_param2',risc0_param2);
            }
        }
        ...
        let mainControlFlowInfo = collectControlFlowInfo(proc, globalNodeMap, globalControlFlowInfo,userParamMap);
            ...
            if (!controlFlowInfo.isEmbedded) {
                ...
                let codeGenerationInfo = {
                    ...
                    concatParameters: (nodeId, isInput, hasType, hasPreviousParameter) => {
                        ...
                        if (node.documentation && node.documentation[0].text && node.documentation[0].text.length > 0) {
                            var localParams = isInput ? extractParameters(node.documentation[0].text, nodeId, null).get('input') : extractParameters(node.documentation[0].text, nodeId, null).get('output');
                            if(localParams[0] == 'string') localParams[0] = 'string memory';
                            if (localParams.length > 0) {
                ...
                let workListGenerationInfo = {
                    ...
                    getParameterType: (nodeId, isType) => {
                        ...
                        if (localParams && localParams.length > 0) {
                            if(localParams[0].type == 'string') localParams[0].type = 'string memory';
